...
enum Mode { RobotShooting, MissionCreate, Maze, BlockLow, BlockHigh, DEBUG, ModeError };
...
Mode getMode()
{
  ...
  if (mode_switch[0] && mode_switch[1] && mode_switch[2] && mode_switch[3] && mode_switch[4]) {
    return DEBUG;
  }

  if (mode_switch[0]) return RobotShooting;
  if (mode_switch[1]) return MissionCreate;
  if (mode_switch[2]) return Maze;
  if (mode_switch[3]) return BlockLow;
  if (mode_switch[4]) return BlockHigh;
